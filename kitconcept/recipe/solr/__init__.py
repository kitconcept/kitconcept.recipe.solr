    ...
    def create_solr_core(self):
        ...
        if self.solr_config:
            ...
            print(
                "copy {}".format(
                    os.path.join(
                        self.buildout["buildout"]["directory"], self.solr_config
                    )
                )
            )
            distutils.dir_util.copy_tree(
                os.path.join(self.buildout["buildout"]["directory"], self.solr_config),
            ...
            requirement = pkg_resources.Requirement.parse("kitconcept.recipe.solr")
...
def solr(options):
    subprocess.call(
        ["./solr"], cwd=options.get("bin-directory") + "/../parts/solr/bin/"
    )
...
def solr_foreground(options):
    ...
    def exit_handler(sig, frame):
        filepath = os.sep.join(
            [options.get("bin-directory"), "solr-{0}.pid".format(options.get("port"))]
        )
...
def solr_restart(options):
    subprocess.call(
        ["./solr", "restart"], cwd=options.get("bin-directory") + "/../parts/solr/bin/"
    )


def solr_stop(options):
    subprocess.call(
        ["./solr", "stop"], cwd=options.get("bin-directory") + "/../parts/solr/bin/"
    )


def solr_status(options):
    subprocess.call(
        ["./solr", "status"], cwd=options.get("bin-directory") + "/../parts/solr/bin/"
    )
